            ...
            {activeView === "reference" ? (
              ...
              <div className="flex-1 min-h-0 flex flex-col">
                <section className="border-b border-border bg-card">
                  <div className="px-4 py-3 flex flex-wrap items-center justify-between gap-4"><div><p className="text-2xs font-medium text-card-foreground">Parser candidates</p><p className="text-2xs text-muted-foreground mt-1">같은 문서 해시의 결과를 한 번에 최대 {DOCUMENT_EVALUATION_MAX_CANDIDATES}개까지 평가합니다.</p></div><button type="button" onClick={evaluateCandidates} disabled={evaluating || selectedGroundTruthSummary?.status !== "frozen" || !selectedCandidateIds.size} className={styles.compactPrimaryButton}>{evaluating ? "Evaluating..." : `Evaluate selected · ${selectedCandidateIds.size}`}</button></div>
