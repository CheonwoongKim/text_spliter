...
function statusClass(status: string): string {
  ...
  if (status === "draft") return "bg-accent/10 text-accent";
  ...
  return (
    <div className="h-full flex bg-surface">
      <aside className="w-[280px] flex-shrink-0 border-r border-border bg-card/25 overflow-y-auto">
        <div className="sticky top-0 z-navigation px-4 py-4 border-b border-border bg-card flex items-center justify-between gap-3">
        ...
        {workspace.benchmarks.map((benchmark) => {
          ...
          return (
            <button key={benchmark.id} type="button" onClick={() => changeBenchmark(benchmark.id)} className={`w-full px-4 py-4 text-left border-b border-border transition-colors ${selectedBenchmarkId === benchmark.id ? "bg-accent/10" : "hover:bg-muted/30"}`}>
              <div className="flex items-start justify-between gap-2"><p className="text-base font-medium text-card-foreground line-clamp-2">{benchmark.name}</p>{truth && <span className={`px-2 py-1 rounded-sm text-xs ${statusClass(truth.status)}`}>{truth.status}</span>}</div>
        ...
        {!workspace.benchmarks.length && <div className="px-4 py-10 text-center"><p className="text-xs text-muted-foreground">저장된 파서 결과에서 첫 기준 문서를 만드세요.</p><button type="button" onClick={openCreate} className="mt-3 text-xs text-accent">New benchmark</button></div>}
      </aside>

      <main className="flex-1 min-w-0 flex flex-col overflow-hidden">
        {error && <div className="px-6 py-3 border-b border-danger-border bg-danger-surface flex items-center justify-between gap-4 text-xs text-danger"><span>{error}</span><button type="button" onClick={() => setError(null)}>Dismiss</button></div>}
        {!selectedBenchmark ? (
          <div className="h-full flex items-center justify-center text-center px-8"><div><p className="text-base font-medium text-card-foreground">문서 파싱 벤치마크가 없습니다</p><p className="text-xs text-muted-foreground mt-2">Document IR을 기준 정답으로 교정한 뒤 같은 원본의 파서 결과를 비교합니다.</p><button type="button" onClick={openCreate} className={`mt-4 ${styles.primaryButton}`}>New benchmark</button></div></div>
        ) : (
          <>
            <header className="px-6 py-4 border-b border-border bg-card/35">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  {benchmarkGroundTruths.length > 1 && <select value={selectedGroundTruthId || ""} onChange={(event) => changeGroundTruth(event.target.value)} disabled={saving || groundTruthLoading} className="h-control-md px-3 border border-border rounded-lg bg-surface text-xs text-card-foreground disabled:opacity-50">{benchmarkGroundTruths.map((item) => <option key={item.id} value={item.id}>Reference v{item.version_number} · {item.status}</option>)}</select>}
            ...
            {activeView === "reference" ? (
              <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-2">
                <section className="min-h-0 border-r border-border flex flex-col">
                  <div className="px-4 py-3 border-b border-border flex items-center justify-between"><div><p className="text-xs font-medium text-card-foreground">Original source</p><p className="text-xs text-muted-foreground mt-1">원본과 기준 IR을 나란히 교정합니다.</p></div>{previewLoading && <span className="text-xs text-muted-foreground">Loading...</span>}</div>
              ...
              <div className="flex-1 min-h-0 flex flex-col">
                <section className="border-b border-border bg-card/15">
                  <div className="px-4 py-3 flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-medium text-card-foreground">Parser candidates</p><p className="text-xs text-muted-foreground mt-1">같은 문서 해시의 결과를 한 번에 최대 {DOCUMENT_EVALUATION_MAX_CANDIDATES}개까지 평가합니다.</p></div><button type="button" onClick={evaluateCandidates} disabled={evaluating || selectedGroundTruthSummary?.status !== "frozen" || !selectedCandidateIds.size} className={styles.compactPrimaryButton}>{evaluating ? "Evaluating..." : `Evaluate selected · ${selectedCandidateIds.size}`}</button></div>
                  <div className="max-h-48 overflow-y-auto border-t border-border">
                    {candidates.map((candidate) => {
                      ...
                      return <label key={candidate.id} className="grid grid-cols-[24px_minmax(0,1fr)_100px_110px] items-center gap-3 px-4 py-3 border-b border-border last:border-b-0 hover:bg-muted/20 cursor-pointer"><input type="checkbox" checked={selectedCandidateIds.has(candidate.id)} onChange={() => toggleCandidate(candidate.id)} className="rounded-sm border-border text-accent focus:ring-accent" /><span className="min-w-0"><span className="block text-xs font-medium text-card-foreground truncate">{candidateLabel(candidate)}</span><span className="block text-xs text-muted-foreground mt-1">{candidate.parser_version || "version unset"}</span></span><span className="text-xs text-muted-foreground text-right">{candidate.processing_time ? `${candidate.processing_time}ms` : "—"}</span><span className="text-right">{latest ? <span className={`px-2 py-1 rounded-sm text-xs ${statusClass(latest.status)}`}>{score(latest.metrics.textF1)}</span> : <span className="text-xs text-muted-foreground">Not evaluated</span>}</span></label>;
                    })}
                ...
                <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)]">
                  <aside className="overflow-y-auto border-r border-border">
                    ...
                    {benchmarkRuns.map((run) => <button key={run.id} type="button" onClick={() => setSelectedRunId(run.id)} className={`w-full text-left px-4 py-3 border-b border-border transition-colors ${selectedRunId === run.id ? "bg-muted/70" : "hover:bg-muted/30"}`}><div className="flex items-start justify-between gap-2"><p className="text-xs font-medium text-card-foreground truncate">{runLabel(run)}</p><span className={`px-2 py-1 rounded-sm text-xs ${statusClass(run.status)}`}>{run.status}</span></div><div className="flex items-center justify-between mt-2 text-xs text-muted-foreground"><span>Text {score(run.metrics.textF1)}</span><span>{run.issue_count} issues</span></div><p className="text-xs text-muted-foreground mt-1">{new Date(run.created_at).toLocaleString()}</p></button>)}
