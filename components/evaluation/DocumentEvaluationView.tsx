    ...
    <div className="h-full flex bg-surface">
      <aside className="w-[280px] flex-shrink-0 border-r border-border-subtle overflow-y-auto">
        <div className="sticky top-0 z-navigation px-4 py-4 border-b border-border-subtle bg-card flex items-center justify-between gap-3">
          <div><p className="text-2xs font-medium text-card-foreground">Document benchmarks</p><p className="text-2xs text-muted-foreground mt-1">{workspace.benchmarks.length} total</p></div>
          <button type="button" onClick={openCreate} className={`${styles.softIconButton} text-base`} title="New document benchmark">+</button>
        </div>
        {workspace.benchmarks.map((benchmark) => {
          ...
          return (
            <button key={benchmark.id} type="button" onClick={() => changeBenchmark(benchmark.id)} className={`w-full px-4 py-4 text-left border-b border-border transition-colors ${selectedBenchmarkId === benchmark.id ? "bg-upload-zone" : "hover:bg-muted"}`}>
              <div className="flex items-start justify-between gap-2"><p className="text-xs font-medium text-card-foreground line-clamp-2">{benchmark.name}</p>{truth && <span className={`px-2 py-1 rounded-sm text-2xs ${statusClass(truth.status)}`}>{truth.status}</span>}</div>
              <p className="text-2xs text-muted-foreground mt-2 truncate">{benchmark.file_name}</p>
              <p className="text-2xs text-muted-foreground mt-2">{runCount} evaluations · {benchmark.attributes.language || "language unset"}</p>
            </button>
          );
        })}
        {!workspace.benchmarks.length && <div className="px-4 py-10 text-center"><p className="text-2xs text-muted-foreground">저장된 파서 결과에서 첫 기준 문서를 만드세요.</p><button type="button" onClick={openCreate} className="mt-3 text-2xs text-card-foreground">New benchmark</button></div>}
      </aside>

      <main className="flex-1 min-w-0 flex flex-col overflow-hidden">
        {error && <div className="px-6 py-3 border-b border-danger-border bg-danger-surface flex items-center justify-between gap-4 text-2xs text-danger"><span>{error}</span><button type="button" onClick={() => setError(null)}>Dismiss</button></div>}
        {!selectedBenchmark ? (
          <div className="h-full flex items-center justify-center text-center px-8"><div><p className="text-xs font-medium text-card-foreground">문서 파싱 벤치마크가 없습니다</p><p className="text-2xs text-muted-foreground mt-2">Document IR을 기준 정답으로 교정한 뒤 같은 원본의 파서 결과를 비교합니다.</p><button type="button" onClick={openCreate} className={`mt-4 ${styles.primaryButton}`}>New benchmark</button></div></div>
        ) : (
          <>
            <header className="px-6 py-4 border-b border-border bg-card">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0"><div className="flex items-center gap-2"><h3 className="text-xs font-semibold text-card-foreground truncate">{selectedBenchmark.name}</h3>{selectedGroundTruthSummary && <span className={`px-2 py-1 rounded-full text-2xs ${statusClass(selectedGroundTruthSummary.status)}`}>v{selectedGroundTruthSummary.version_number} · {selectedGroundTruthSummary.status}</span>}</div><p className="text-2xs text-muted-foreground mt-1 truncate">{selectedBenchmark.file_name} · {selectedBenchmark.document_hash || "hash unavailable"}</p></div>
                <div className="flex items-center gap-2">
                  {benchmarkGroundTruths.length > 1 && <select value={selectedGroundTruthId || ""} onChange={(event) => changeGroundTruth(event.target.value)} disabled={saving || groundTruthLoading} className="h-control-md px-3 border border-border rounded-lg bg-surface text-2xs text-card-foreground disabled:opacity-50">{benchmarkGroundTruths.map((item) => <option key={item.id} value={item.id}>Reference v{item.version_number} · {item.status}</option>)}</select>}
                  {selectedGroundTruthSummary?.status === "frozen" && <Button variant="outline" size="sm" onClick={cloneReference} disabled={saving}>Create next version</Button>}
                  <Button variant="dangerGhost" size="sm" onClick={deleteBenchmark} disabled={saving}>Delete</Button>
                </div>
              </div>
              <nav className="flex items-center gap-4 mt-4 -mb-4">
                <button type="button" onClick={() => setActiveView("runs")} className={`pb-3 text-2xs font-medium border-b-2 ${activeView === "runs" ? "border-surface-foreground text-card-foreground" : "border-transparent text-muted-foreground"}`}>Candidates & results</button>
                <button type="button" onClick={() => setActiveView("reference")} className={`pb-3 text-2xs font-medium border-b-2 ${activeView === "reference" ? "border-surface-foreground text-card-foreground" : "border-transparent text-muted-foreground"}`}>Source & reference IR</button>
              </nav>
            </header>
            ...
            {activeView === "reference" ? (
              <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-2">
                <section className="min-h-0 border-r border-border flex flex-col">
                  <div className="px-4 py-3 border-b border-border flex items-center justify-between"><div><p className="text-2xs font-medium text-card-foreground">Original source</p><p className="text-2xs text-muted-foreground mt-1">원본과 기준 IR을 나란히 교정합니다.</p></div>{previewLoading && <span className="text-2xs text-muted-foreground">Loading...</span>}</div>
                  <div className="flex-1 min-h-0 bg-upload-zone">
                    {previewUrl && previewType?.startsWith("image/") ? <div className="relative w-full h-full"><Image src={previewUrl} alt={selectedBenchmark.file_name} fill unoptimized sizes="50vw" className="object-contain" /></div> : previewUrl && previewType?.startsWith("application/pdf") ? <iframe src={previewUrl} title={selectedBenchmark.file_name} className="w-full h-full border-0" /> : <div className="h-full flex items-center justify-center px-8 text-center text-2xs text-muted-foreground">{selectedBenchmark.source_storage_key ? "이 파일 형식은 인라인 미리보기를 지원하지 않습니다." : "원본 Storage 객체가 연결되지 않았습니다."}</div>}
                  </div>
                </section>
                <section className="min-h-0 flex flex-col">
                  <div className="px-4 py-3 border-b border-border flex flex-wrap items-center justify-between gap-3"><div><p className="text-2xs font-medium text-card-foreground">Reference Document IR</p><p className="text-2xs text-muted-foreground mt-1">블록 텍스트·유형·순서·영역·표 셀을 검수합니다.</p></div><div className="flex items-center gap-2">{groundTruthLoading ? <span className="text-2xs text-muted-foreground">Loading reference...</span> : selectedGroundTruthSummary?.status === "draft" ? <><Button variant="outline" size="sm" onClick={saveReference} disabled={saving || !referenceDirty || !referenceEditable}>{saving ? "Saving..." : "Save draft"}</Button><Button variant="primary" size="sm" onClick={freezeReference} disabled={saving || referenceDirty || !referenceEditable}>Freeze reference</Button></> : <span className="text-2xs text-muted-foreground">Frozen {selectedGroundTruthSummary?.frozen_at ? new Date(selectedGroundTruthSummary.frozen_at).toLocaleString() : ""}</span>}</div></div>
                  <textarea value={referenceJson} onChange={(event) => { setReferenceJson(event.target.value); setReferenceDirty(true); }} readOnly={!referenceEditable} placeholder={groundTruthLoading ? "Loading reference Document IR..." : undefined} spellCheck={false} className="flex-1 min-h-[360px] resize-none p-4 bg-surface text-2xs leading-5 font-mono text-card-foreground focus:outline-none read-only:text-muted-foreground" />
                  <div className="p-4 border-t border-border"><label className="block"><span className="block text-2xs text-muted-foreground mb-2">Reference notes</span><textarea value={referenceNotes} onChange={(event) => { setReferenceNotes(event.target.value); setReferenceDirty(true); }} readOnly={!referenceEditable} rows={2} className={styles.textArea} /></label>{referenceDirty && <p className="text-2xs text-warning mt-2">저장되지 않은 변경사항이 있습니다. 고정 전에 초안을 저장하세요.</p>}</div>
                </section>
              </div>
            ) : (
              <div className="flex-1 min-h-0 flex flex-col">
                <section className="border-b border-border bg-card">
                  <div className="px-4 py-3 flex flex-wrap items-center justify-between gap-4"><div><p className="text-2xs font-medium text-card-foreground">Parser candidates</p><p className="text-2xs text-muted-foreground mt-1">같은 문서 해시의 결과를 한 번에 최대 {DOCUMENT_EVALUATION_MAX_CANDIDATES}개까지 평가합니다.</p></div><Button variant="primary" size="sm" onClick={evaluateCandidates} disabled={evaluating || selectedGroundTruthSummary?.status !== "frozen" || !selectedCandidateIds.size}>{evaluating ? "Evaluating..." : `Evaluate selected · ${selectedCandidateIds.size}`}</Button></div>
                  <div className="max-h-48 overflow-y-auto border-t border-border">
                    {candidates.map((candidate) => {
                      ...
                      return <label key={candidate.id} className="grid grid-cols-[24px_minmax(0,1fr)_100px_110px] items-center gap-3 px-4 py-3 border-b border-border last:border-b-0 hover:bg-muted cursor-pointer"><input type="checkbox" checked={selectedCandidateIds.has(candidate.id)} onChange={() => toggleCandidate(candidate.id)} className="rounded-sm border-border accent-surface-foreground focus:outline-none focus-visible:ring-1 focus-visible:ring-surface-foreground" /><span className="min-w-0"><span className="block text-2xs font-medium text-card-foreground truncate">{candidateLabel(candidate)}</span><span className="block text-2xs text-muted-foreground mt-1">{candidate.parser_version || "version unset"}</span></span><span className="text-2xs text-muted-foreground text-right">{candidate.processing_time ? `${candidate.processing_time}ms` : "—"}</span><span className="text-right">{latest ? <span className={`px-2 py-1 rounded-sm text-2xs ${statusClass(latest.status)}`}>{score(latest.metrics.textF1)}</span> : <span className="text-2xs text-muted-foreground">Not evaluated</span>}</span></label>;
                    })}
                    {!candidates.length && <div className="px-4 py-6 text-center text-2xs text-muted-foreground">같은 원본에서 생성된 다른 파서 실행이 없습니다.</div>}
                  </div>
                  {selectedGroundTruthSummary?.status !== "frozen" && <div className="px-4 py-3 border-t border-warning-border bg-warning-surface text-2xs text-warning">평가 전에 Reference IR을 검수하고 고정하세요.</div>}
                </section>
                ...
                <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)]">
                  <aside className="overflow-y-auto border-r border-border">
                    <div className="px-4 py-3 border-b border-border text-2xs uppercase tracking-wide text-muted-foreground">Evaluation history</div>
                    {benchmarkRuns.map((run) => <button key={run.id} type="button" onClick={() => setSelectedRunId(run.id)} className={`w-full text-left px-4 py-3 border-b border-border transition-colors ${selectedRunId === run.id ? "bg-upload-zone" : "hover:bg-muted"}`}><div className="flex items-start justify-between gap-2"><p className="text-2xs font-medium text-card-foreground truncate">{runLabel(run)}</p><span className={`px-2 py-1 rounded-sm text-2xs ${statusClass(run.status)}`}>{run.status}</span></div><div className="flex items-center justify-between mt-2 text-2xs text-muted-foreground"><span>Text {score(run.metrics.textF1)}</span><span>{run.issue_count} issues</span></div><p className="text-2xs text-muted-foreground mt-1">{new Date(run.created_at).toLocaleString()}</p></button>)}
                    {!benchmarkRuns.length && <div className="px-4 py-8 text-center text-2xs text-muted-foreground">평가 이력이 없습니다.</div>}
                  </aside>

                  <section className="overflow-y-auto">
                    {!selectedRunSummary || !runDetail || runDetail.id !== selectedRunSummary.id ? <div className="h-full flex items-center justify-center text-2xs text-muted-foreground">평가 실행을 선택하세요.</div> : <div className="max-w-6xl mx-auto px-6 py-6 space-y-8">
                      <div><div className="flex flex-wrap items-start justify-between gap-4"><div><h4 className="text-xs font-semibold text-card-foreground">{runLabel(selectedRunSummary)}</h4><p className="text-2xs text-muted-foreground mt-1">{runDetail.framework_version} · reference v{benchmarkGroundTruths.find((item) => item.id === runDetail.ground_truth_id)?.version_number || "—"}</p></div><span className="text-2xs text-muted-foreground">{runDetail.metrics.samples.matchedBlocks}/{runDetail.metrics.samples.referenceBlocks} blocks matched</span></div>
                        <div className="flex flex-wrap mt-4 border-y border-border">{METRICS.map((metric) => <button key={metric.key} type="button" onClick={() => { setIssueDimension(metric.dimension); const first = runDetail.issues.findIndex((issue) => metric.dimension === "all" || issue.dimension === metric.dimension); if (first >= 0) setSelectedIssueIndex(first); }} className={`flex-[1_1_125px] min-w-[125px] text-left py-3 pr-4 transition-colors ${issueDimension === metric.dimension ? "text-card-foreground" : "hover:text-card-foreground"}`}><span className="block text-2xs uppercase tracking-wide text-muted-foreground">{metric.label}</span><strong className="block text-xs mt-1 text-card-foreground">{score(runDetail.metrics[metric.key])}</strong></button>)}</div>
                      </div>

                      <div className="grid grid-cols-1 2xl:grid-cols-[minmax(0,1fr)_320px] gap-8">
                        <div>
                          <div className="flex items-center justify-between gap-3 mb-3"><div><h5 className="text-2xs font-medium text-card-foreground">Issues</h5><p className="text-2xs text-muted-foreground mt-1">블록 매칭 이후의 누락·추가·순서·영역 오차입니다.</p></div><select value={issueDimension} onChange={(event) => { const dimension = event.target.value as DocumentEvaluationDimension | "all"; setIssueDimension(dimension); const first = runDetail.issues.findIndex((issue) => dimension === "all" || issue.dimension === dimension); setSelectedIssueIndex(Math.max(0, first)); }} className="h-8 px-2 border border-border rounded-lg bg-surface text-2xs text-card-foreground">{Object.entries(DIMENSION_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
                          <div className="border-t border-border max-h-[420px] overflow-y-auto">{issueEntries.map(({ issue, index }) => <button key={`${index}-${issue.code}`} type="button" onClick={() => setSelectedIssueIndex(index)} className={`w-full grid grid-cols-[70px_90px_minmax(0,1fr)_52px] gap-3 px-3 py-3 text-left border-b border-border transition-colors ${selectedIssueIndex === index ? "bg-upload-zone" : "hover:bg-muted"}`}><span className={`text-2xs uppercase ${issue.severity === "error" ? "text-danger" : issue.severity === "warning" ? "text-warning" : "text-muted-foreground"}`}>{issue.severity}</span><span className="text-2xs text-muted-foreground">p.{issue.pageNumber} · {issue.dimension}</span><span className="text-2xs text-card-foreground">{issue.message}</span><span className="text-2xs text-right text-muted-foreground">{score(issue.score)}</span></button>)}{!issueEntries.length && <div className="px-4 py-8 text-center text-2xs text-muted-foreground">이 범주의 오차가 없습니다.</div>}</div>
                        </div>
                        <div><h5 className="text-2xs font-medium text-card-foreground mb-3">Page summary</h5><div className="border-t border-border">{runDetail.metrics.pages.map((page) => <div key={page.pageNumber} className="grid grid-cols-[40px_1fr_1fr_1fr] gap-2 py-3 border-b border-border text-2xs"><span className="text-card-foreground">p.{page.pageNumber}</span><span className="text-muted-foreground">Text <strong className="text-card-foreground">{score(page.textSimilarity)}</strong></span><span className="text-muted-foreground">Order <strong className="text-card-foreground">{score(page.readingOrderAccuracy)}</strong></span><span className="text-muted-foreground text-right">{page.issueCount} issues</span></div>)}</div></div>
                      </div>

                      {selectedIssue && <section className="border-t border-border pt-6"><div className="flex items-center justify-between gap-3 mb-4"><div><h5 className="text-2xs font-medium text-card-foreground">Reference / candidate inspection</h5><p className="text-2xs text-muted-foreground mt-1">{selectedIssue.code} · page {selectedIssue.pageNumber}</p></div>{typeof selectedIssue.score === "number" && <span className="text-2xs font-semibold text-card-foreground">{score(selectedIssue.score)}</span>}</div><div className="grid grid-cols-1 xl:grid-cols-2 gap-6"><div><p className="text-2xs uppercase tracking-wide text-muted-foreground mb-2">Reference block</p><pre className="min-h-32 max-h-72 overflow-auto p-4 border-l-2 border-surface-foreground bg-upload-zone text-2xs leading-5 text-card-foreground whitespace-pre-wrap">{blockContent(referenceBlock)}</pre></div><div><p className="text-2xs uppercase tracking-wide text-muted-foreground mb-2">Candidate block</p><pre className="min-h-32 max-h-72 overflow-auto p-4 border-l-2 border-border bg-upload-zone text-2xs leading-5 text-card-foreground whitespace-pre-wrap">{blockContent(candidateBlock)}</pre></div></div></section>}
                    </div>}
                  </section>
                </div>
              </div>
      ...
      {createOpen && <div className="fixed inset-0 z-modal bg-overlay flex items-center justify-center p-4"><div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-card border border-border rounded-lg shadow-lg p-6"><h3 className="text-base font-semibold text-card-foreground">New document benchmark</h3><p className="text-2xs text-muted-foreground mt-1">저장된 파서 결과를 교정 가능한 Reference Document IR 초안으로 복제합니다.</p><div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6"><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Reference parser run</span><select value={createSourceId} onChange={(event) => { setCreateSourceId(event.target.value); const source = benchmarkSources.find((item) => item.id === Number(event.target.value)); if (source) setCreateName(`${source.file_name} reference`); }} className={styles.field}><option value="">Select a stored Document IR</option>{benchmarkSources.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.file_name} · {candidateLabel(candidate)}</option>)}</select>{!benchmarkSources.length && <span className="block text-2xs text-warning mt-2">유효한 문서 해시가 없습니다. 문서를 다시 파싱한 뒤 기준 문서를 만드세요.</span>}</label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Name</span><input value={createName} onChange={(event) => setCreateName(event.target.value)} className={styles.field} /></label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Description</span><textarea value={createDescription} onChange={(event) => setCreateDescription(event.target.value)} rows={3} className={styles.textArea} /></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Document type</span><input value={createDocumentType} onChange={(event) => setCreateDocumentType(event.target.value)} className={styles.field} placeholder="financial-report" /></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Language</span><input value={createLanguage} onChange={(event) => setCreateLanguage(event.target.value)} className={styles.field} placeholder="ko" /></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Layout</span><input value={createLayout} onChange={(event) => setCreateLayout(event.target.value)} className={styles.field} placeholder="multi-column" /></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Source quality</span><input value={createQuality} onChange={(event) => setCreateQuality(event.target.value)} className={styles.field} placeholder="digital / scan" /></label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Tags</span><input value={createTags} onChange={(event) => setCreateTags(event.target.value)} className={styles.field} placeholder="table, chart, rotated" /></label></div><div className="flex justify-end gap-3 mt-6"><Button variant="ghost" size="sm" onClick={() => setCreateOpen(false)} disabled={saving}>Cancel</Button><Button variant="primary" size="md" onClick={createBenchmark} disabled={saving || !createSourceId || !createName.trim()}>{saving ? "Creating..." : "Create draft reference"}</Button></div></div></div>}
    </div>
  );
}
