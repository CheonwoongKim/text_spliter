  ...
  return (
    <div className="h-full flex bg-surface">
      ...
      <Modal
        ...
        size="xl"
        footer={<><Button variant="ghost" size="sm" onClick={() => setCreateOpen(false)} disabled={saving}>Cancel</Button><Button variant="primary" size="md" onClick={createBenchmark} disabled={saving || !createSourceId || !createName.trim()}>{saving ? "Creating..." : "Create draft reference"}</Button></>}
      ><div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6"><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Reference parser run</span><Select fieldSize="lg" value={createSourceId} onChange={(event) => { setCreateSourceId(event.target.value); const source = benchmarkSources.find((item) => item.id === Number(event.target.value)); if (source) setCreateName(`${source.file_name} reference`); }}><option value="">Select a stored Document IR</option>{benchmarkSources.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.file_name} · {candidateLabel(candidate)}</option>)}</Select>{!benchmarkSources.length && <span className="block text-2xs text-warning mt-2">유효한 문서 해시가 없습니다. 문서를 다시 파싱한 뒤 기준 문서를 만드세요.</span>}</label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Name</span><Input fieldSize="lg" value={createName} onChange={(event) => setCreateName(event.target.value)}/></label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Description</span><Textarea value={createDescription} onChange={(event) => setCreateDescription(event.target.value)} rows={3}/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Document type</span><Input fieldSize="lg" value={createDocumentType} onChange={(event) => setCreateDocumentType(event.target.value)} placeholder="financial-report"/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Language</span><Input fieldSize="lg" value={createLanguage} onChange={(event) => setCreateLanguage(event.target.value)} placeholder="ko"/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Layout</span><Input fieldSize="lg" value={createLayout} onChange={(event) => setCreateLayout(event.target.value)} placeholder="multi-column"/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Source quality</span><Input fieldSize="lg" value={createQuality} onChange={(event) => setCreateQuality(event.target.value)} placeholder="digital / scan"/></label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Tags</span><Input fieldSize="lg" value={createTags} onChange={(event) => setCreateTags(event.target.value)} placeholder="table, chart, rotated"/></label></div></Modal>
    </div>
  );
}
