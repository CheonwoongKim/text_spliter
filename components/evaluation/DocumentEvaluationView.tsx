  ...
  return (
    <div className="h-full flex bg-surface">
      <aside className="w-[280px] flex-shrink-0 border-r border-border-subtle overflow-y-auto">
        ...
        {!workspace.benchmarks.length && <div className="px-4 py-10 text-center"><p className="text-2xs text-muted-foreground">저장된 파서 결과에서 첫 기준 문서를 만드세요.</p><button type="button" onClick={openCreate} className="mt-3 text-2xs text-card-foreground">새 벤치마크</button></div>}
      </aside>

      <main className="flex-1 min-w-0 flex flex-col overflow-hidden">
        {error && <div className="px-6 py-3 border-b border-danger-border bg-danger-surface flex items-center justify-between gap-4 text-2xs text-danger"><span>{error}</span><button type="button" onClick={() => setError(null)}>닫기</button></div>}
        {!selectedBenchmark ? (
            ...
            {activeView === "reference" ? (
              <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-2">
                <section className="min-h-0 border-r border-border flex flex-col">
                  <div className="px-4 py-3 border-b border-border flex items-center justify-between"><div><p className="text-2xs font-medium text-card-foreground">Original source</p><p className="text-2xs text-muted-foreground mt-1">원본과 기준 IR을 나란히 교정합니다.</p></div>{previewLoading && <span className="text-2xs text-muted-foreground">불러오는 중...</span>}</div>
      ...
      ><div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6"><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Reference parser run</span><Select fieldSize="lg" value={createSourceId} onChange={(event) => { setCreateSourceId(event.target.value); const source = benchmarkSources.find((item) => item.id === Number(event.target.value)); if (source) setCreateName(`${source.file_name} reference`); }}><option value="">Select a stored Document IR</option>{benchmarkSources.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.file_name} · {candidateLabel(candidate)}</option>)}</Select>{!benchmarkSources.length && <span className="block text-2xs text-warning mt-2">유효한 문서 해시가 없습니다. 문서를 다시 파싱한 뒤 기준 문서를 만드세요.</span>}</label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">이름</span><Input fieldSize="lg" value={createName} onChange={(event) => setCreateName(event.target.value)}/></label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">설명</span><Textarea value={createDescription} onChange={(event) => setCreateDescription(event.target.value)} rows={3}/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Document type</span><Input fieldSize="lg" value={createDocumentType} onChange={(event) => setCreateDocumentType(event.target.value)} placeholder="financial-report"/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">언어</span><Input fieldSize="lg" value={createLanguage} onChange={(event) => setCreateLanguage(event.target.value)} placeholder="ko"/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Layout</span><Input fieldSize="lg" value={createLayout} onChange={(event) => setCreateLayout(event.target.value)} placeholder="multi-column"/></label><label><span className="block text-2xs font-medium text-muted-foreground mb-2">Source quality</span><Input fieldSize="lg" value={createQuality} onChange={(event) => setCreateQuality(event.target.value)} placeholder="digital / scan"/></label><label className="block md:col-span-2"><span className="block text-2xs font-medium text-muted-foreground mb-2">Tags</span><Input fieldSize="lg" value={createTags} onChange={(event) => setCreateTags(event.target.value)} placeholder="table, chart, rotated"/></label></div></Modal>
    </div>
  );
}
